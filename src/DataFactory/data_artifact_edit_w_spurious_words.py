...
patchHorizRoot2= join(htrAssetsRoot, 'cropped_patches', 'irs_horizontal')
patchVerticRoot=join(htrAssetsRoot,'cropped_patches','nw_vertical-2')
patchBoxesFiles = glob(join(patchBoxesRoot, '*.jpg'))
patchHorizFiles = glob(join(patchHorizRoot, '*.jpg'))
patchHorizFiles2 = glob(join(patchHorizRoot2, '*.png'))
patchVerticFiles = glob(join(patchVerticRoot, '*.jpg'))
...
def merge_patch_horiz_random2(img, centroid_std=.05):
  imgSize = img.shape[::-1]
  #imPatchFile = choice(patchBoxesFiles)
  if np.random.rand()<0.95:
    imPatchFile = choice(patchHorizFiles2)
    imPatch = cv2.imread(imPatchFile, cv2.IMREAD_GRAYSCALE)
    imPatch = cv2.resize(imPatch, None, fx=1, fy=0.5)    
  else:
    imPatchFile = choice(patchHorizFiles)
    imPatch = cv2.imread(imPatchFile, cv2.IMREAD_GRAYSCALE)
    imPatch = cv2.resize(imPatch, None, fx=4, fy=1)
  ...
  #print(imPatch.shape)
  if np.random.rand()<0.95:
    centroid = [imgSize[1] * (.85 + normal(0, centroid_std)/2), imgSize[0] / 2 * (1 + normal(0, centroid_std))]
  else:
    centroid = [imgSize[1] * (.15 + normal(0, centroid_std)/2), imgSize[0] / 2 * (1 + normal(0, centroid_std))]
  #print(str(img.shape)+'!')
  #print(str(imPatch.shape)+'!')
  return merge_patch(img, imPatch, centroid, threshold=50)

def merge_patch_vertic_random(img, centroid_std=.05):
  imgSize = img.shape[::-1]
  #imPatchFile = choice(patchBoxesFiles)
  imPatchFile = choice(patchVerticFiles)
  imPatch = cv2.imread(imPatchFile, cv2.IMREAD_GRAYSCALE)
  imPatch = cv2.resize(imPatch, None, fx=1, fy=1) # tricky
  imPatch = cv2.normalize(imPatch, None, np.min(img), np.max(img), norm_type=cv2.NORM_MINMAX)
  #print(imPatch.shape)
  centroid = [imgSize[1] / 2 * (1 + normal(0, centroid_std)),imgSize[0] * (np.random.choice([0.02,0.95]) + normal(0, centroid_std)/2)]
  #print(str(img.shape)+'!')
  #print(str(imPatch.shape)+'!')
  return merge_patch(img, imPatch, centroid, threshold=50)

def merge_patch_words_random(img, patchSpuWordsFiles, centroid_std=.05):
  #test_dir='/root/datasets/artifact_images_no_intersect/artifact_images_no_intersect/'
  imgSize = img.shape[::-1]
  imPatchFile = choice(patchSpuWordsFiles)
  imPatch = cv2.imread(imPatchFile, cv2.IMREAD_GRAYSCALE)
  while imPatch is None or np.min(imPatch.shape) <= 1:
        imPatchFile = choice(patchSpuWordsFiles)
        imPatch = cv2.imread(imPatchFile, cv2.IMREAD_GRAYSCALE)
  #remove margins  
  np.sum(255-imPatch,axis=1)  
  y_non_zeros=np.where(np.sum(255-imPatch,axis=1) >0)
  imPatch=imPatch[np.min(y_non_zeros):np.max(y_non_zeros)+1,:]
  #cv2.imwrite(test_dir+'spuw_patch_orig.jpg',imPatch)
  imPatch = cv2.resize(imPatch, img.shape[::-1])
  imPatch = cv2.normalize(imPatch, None, np.min(img), np.max(img), norm_type=cv2.NORM_MINMAX)

  #cv2.imwrite(test_dir+'spuw_patch.jpg',imPatch) #-----
  hc=choice((uniform(low=0.15, high=0.4, size=None)-0.5,
uniform(low=0.6, high=0.85, size=None)+0.5))

  centroid = [imgSize[1] *hc, imgSize[0] / 2 * (1 + normal(0, centroid_std))]
  return merge_patch(img, imPatch, centroid, threshold=50)
